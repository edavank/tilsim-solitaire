// Multi-language word pools for Tılsım Solitaire

export const WORD_POOLS = {
  tr: [
    { name: 'Meyveler', words: ['Elma', 'Armut', 'Kiraz', 'Portakal', 'Çilek', 'Muz', 'Karpuz', 'Üzüm', 'Şeftali', 'Ananas', 'Nar', 'İncir'] },
    { name: 'Hayvanlar', words: ['Kedi', 'Köpek', 'Kuş', 'Kaplan', 'Tavşan', 'At', 'Kaplumbağa', 'Kurbağa', 'Aslan', 'Fil', 'Zürafa', 'Penguen'] },
    { name: 'Renkler', words: ['Kırmızı', 'Mavi', 'Yeşil', 'Sarı', 'Turuncu', 'Mor', 'Pembe', 'Beyaz'] },
    { name: 'Sporlar', words: ['Futbol', 'Basketbol', 'Tenis', 'Yüzme', 'Voleybol', 'Boks', 'Bisiklet', 'Koşu'] },
    { name: 'İçecekler', words: ['Maden suyu', 'Kahve', 'Su', 'Süt', 'Limonata', 'Ayran', 'Meyve suyu', 'Smoothie'] },
    { name: 'Ağaçlar', words: ['Çam', 'Meşe', 'Kavak', 'Ceviz', 'Zeytin', 'Palmiye'] },
    { name: 'Müzik', words: ['Gitar', 'Piyano', 'Davul', 'Keman', 'Flüt', 'Saz'] },
    { name: 'Sebzeler', words: ['Domates', 'Biber', 'Havuç', 'Patates', 'Soğan', 'Patlıcan', 'Brokoli', 'Mısır'] },
    { name: 'Ülkeler', words: ['Türkiye', 'Japonya', 'Brezilya', 'İtalya', 'Fransa', 'Almanya'] },
    { name: 'Gezegenler', words: ['Mars', 'Jüpiter', 'Satürn', 'Venüs', 'Merkür', 'Neptün'] },
    { name: 'Mevsimler', words: ['İlkbahar', 'Yaz', 'Sonbahar', 'Kış'] },
    { name: 'Duygular', words: ['Mutluluk', 'Üzüntü', 'Şaşkınlık', 'Korku', 'Öfke', 'Heyecan'] },
    { name: 'Okul', words: ['Kalem', 'Defter', 'Silgi', 'Cetvel', 'Kitap', 'Sırt çantası'] },
    { name: 'Yiyecekler', words: ['Pizza', 'Hamburger', 'Sushi', 'Makarna', 'Çorba', 'Pilav'] },
    { name: 'Hava durumu', words: ['Güneşli', 'Yağmurlu', 'Karlı', 'Bulutlu', 'Fırtınalı', 'Sisli'] },
    { name: 'Ulaşım', words: ['Araba', 'Otobüs', 'Tren', 'Uçak', 'Metro'] },
    { name: 'Giysiler', words: ['Gömlek', 'Pantolon', 'Şapka', 'Ayakkabı', 'Elbise', 'Çorap'] },
    { name: 'Kuşlar', words: ['Papağan', 'Serçe', 'Kartal', 'Martı', 'Baykuş', 'Flamingo'] },
    // Yeni kategoriler
    { name: 'Çiçekler', words: ['Gül', 'Papatya', 'Lale', 'Menekşe', 'Orkide', 'Sümbül'] },
    { name: 'Deniz', words: ['Yunus', 'Köpekbalığı', 'Ahtapot', 'Yengeç', 'Denizatı', 'İstakoz'] },
    { name: 'Meslekler', words: ['Doktor', 'Öğretmen', 'Mühendis', 'Aşçı', 'Pilot', 'Polis'] },
    { name: 'Mobilya', words: ['Masa', 'Sandalye', 'Dolap', 'Koltuk', 'Yatak', 'Ayna'] },
    { name: 'Vücut', words: ['Göz', 'Burun', 'Kulak', 'El', 'Ayak', 'Kalp'] },
    { name: 'Uzay', words: ['Ay', 'Güneş', 'Yıldız', 'Galaksi', 'Meteor', 'Uydu'] },
    { name: 'Şehirler', words: ['Konya', 'Ankara', 'İzmir', 'Antalya', 'Bursa', 'Trabzon'] },
    { name: 'Takılar', words: ['Yüzük', 'Kolye', 'Bilezik', 'Küpe', 'Broş', 'Saat'] },
    { name: 'Tatlılar', words: ['Baklava', 'Künefe', 'Sütlaç', 'Helva', 'Lokum', 'Kadayıf'] },
    { name: 'Baharat', words: ['Tuz', 'Pul biber', 'Kimyon', 'Kekik', 'Tarçın', 'Safran'] },
    // Yeni çeşitli kategoriler
    { name: 'Teknoloji', words: ['Telefon', 'Tablet', 'Laptop', 'Robot', 'Drone', 'Yazıcı'] },
    { name: 'Bilgisayar', words: ['Klavye', 'Mouse', 'Ekran', 'İşlemci', 'Bellek', 'Disk'] },
    { name: 'Sinema', words: ['Film', 'Yönetmen', 'Aktör', 'Sahne', 'Senaryo', 'Kamera'] },
    { name: 'Mutfak', words: ['Tencere', 'Tava', 'Bıçak', 'Tabak', 'Bardak', 'Kaşık'] },
    { name: 'Doğa', words: ['Dağ', 'Çayır', 'Nehir', 'Göl', 'Şelale', 'Mağara'] },
    { name: 'Tatil', words: ['Havuz', 'Otel', 'Bavul', 'Pasaport', 'Güneş kremi', 'Harita'] },
    { name: 'Oyuncaklar', words: ['Kukla', 'Misket', 'Lego', 'Puzzle', 'Top', 'Uçurtma'] },
    { name: 'Matematik', words: ['Toplama', 'Çıkarma', 'Çarpma', 'Bölme', 'Üçgen', 'Daire'] },
    { name: 'Sanat', words: ['Resim', 'Heykel', 'Fırça', 'Tuval', 'Boya', 'Müze'] },
    { name: 'Araçlar', words: ['Çekiç', 'Tornavida', 'Pense', 'Testere', 'Matkap', 'Anahtar'] },
    { name: 'Kırtasiye', words: ['Makas', 'Yapıştırıcı', 'Bant', 'Zımba', 'Dosya', 'Klasör'] },
    { name: 'Süper Kahramanlar', words: ['Maske', 'Pelerin', 'Güç', 'Kalkan', 'Kılıç', 'Zırh'] },
    // 20 yeni kategori
    { name: 'Kahvaltı', words: ['Peynir', 'Simit', 'Bal', 'Yumurta', 'Reçel', 'Tereyağı'] },
    { name: 'Türk Mutfağı', words: ['Kebap', 'Lahmacun', 'Pide', 'Mantı', 'Börek', 'Dolma'] },
    { name: 'Masal', words: ['Prenses', 'Ejderha', 'Büyücü', 'Şövalye', 'Hazine', 'Kale'] },
    { name: 'Deniz Araçları', words: ['Tekne', 'Yat', 'Feribot', 'Kayık', 'Sürat', 'Yelkenli'] },
    { name: 'Kış Sporları', words: ['Kayak', 'Buz pateni', 'Hokey', 'Snowboard', 'Kızak', 'Curling'] },
    { name: 'Süt Ürünleri', words: ['Yoğurt', 'Lor', 'Krema', 'Süzme', 'Kaymak', 'Kefir'] },
    { name: 'Bina', words: ['Kapı', 'Pencere', 'Çatı', 'Merdiven', 'Balkon', 'Asansör'] },
    { name: 'Orman', words: ['Mantar', 'Yaprak', 'Sincap', 'Geyik', 'Tilki', 'Kurt'] },
    { name: 'Plaj', words: ['Kum', 'Dalga', 'Şemsiye', 'Deniz yıldızı', 'Kabuk', 'Palet'] },
    { name: 'Müzik Türleri', words: ['Pop', 'Rock', 'Caz', 'Klasik', 'Rap', 'Blues'] },
    { name: 'Trafik', words: ['Işık', 'Tabela', 'Yol', 'Kavşak', 'Köprü', 'Tünel'] },
    { name: 'Market', words: ['Sepet', 'Kasa', 'Raf', 'Etiket', 'Poşet', 'Fatura'] },
    { name: 'Hastane', words: ['Hemşire', 'İlaç', 'Ameliyat', 'Muayene', 'Röntgen', 'Ambulans'] },
    { name: 'Oyunlar', words: ['Satranç', 'Dama', 'Tavla', 'Bilardo', 'Dart', 'Bowling'] },
    { name: 'Gemi', words: ['Kaptan', 'Dümen', 'Çapa', 'Güverte', 'Pusula', 'Yelken'] },
    { name: 'Sabah', words: ['Alarm', 'Tost', 'Duş', 'Diş fırçası', 'Gazete', 'Erken'] },
    { name: 'Ağırlık Birimleri', words: ['Gram', 'Kilogram', 'Ton', 'Ons', 'Miligram', 'Libre'] },
    { name: 'Partisyon', words: ['Nota', 'Akor', 'Ritim', 'Melodi', 'Tempo', 'Armoni'] },
    { name: 'Simya', words: ['Altın', 'Gümüş', 'Bakır', 'Demir', 'Elmas', 'Kristal'] },
    { name: 'Fitness', words: ['Burpee', 'Plank', 'Squat', 'Dambıl', 'Pilates', 'Kardiyo'] },
    // === YENİ KATEGORİLER (30 adet) ===
    { name: 'Böcekler', words: ['Kelebek', 'Karınca', 'Arı', 'Uğur böceği', 'Ateş böceği', 'Çekirge'] },
    { name: 'Kamp', words: ['Çadır', 'Uyku tulumu', 'Matara', 'Fener', 'Kamp ateşi', 'Balta'] },
    { name: 'Fotoğrafçılık', words: ['Objektif', 'Flaş', 'Poz', 'Kadraj', 'Deklanşör', 'Tripod'] },
    { name: 'Astroloji', words: ['Koç', 'Boğa', 'İkizler', 'Başak', 'Aslan burcu', 'Akrep'] },
    { name: 'Mitoloji', words: ['Zeus', 'Athena', 'Poseidon', 'Hades', 'Apollon', 'Artemis'] },
    { name: 'Pasta', words: ['Fondan', 'Pandispanya', 'Vanilya', 'Muffin', 'Kurabiye', 'Brownie'] },
    { name: 'Düğün', words: ['Gelin', 'Damat', 'Nikah', 'Düvak', 'Davetiye', 'Dans'] },
    { name: 'Bebek', words: ['Emzik', 'Biberon', 'Bez', 'Beşik', 'Mama', 'Kundak'] },
    { name: 'Bahçe', words: ['Tohum', 'Toprak', 'Gübre', 'Sulama', 'Kürek', 'Sera'] },
    { name: 'Enstrüman', words: ['Ud', 'Ney', 'Kanun', 'Tanbur', 'Zurna', 'Bağlama'] },
    { name: 'Yazılım', words: ['Kod', 'Hata', 'Sunucu', 'Veritabanı', 'Algoritma', 'Arayüz'] },
    { name: 'Coğrafya', words: ['Vadi', 'Plato', 'Yarımada', 'Okyanus', 'Çöl', 'Volkan'] },
    { name: 'Tarih', words: ['İmparator', 'Savaş', 'Antlaşma', 'Hanedan', 'Fetih', 'Devrim'] },
    { name: 'Mimari', words: ['Sütun', 'Kemer', 'Kubbe', 'Minare', 'Köşk', 'Revak'] },
    { name: 'Dövüş Sanatları', words: ['Karate', 'Judo', 'Taekwondo', 'Aikido', 'Kung fu', 'Güreş'] },
    { name: 'Çay', words: ['Demlik', 'Fincan', 'Şeker', 'Çaydanlık', 'Poşet çay', 'Samovar'] },
    { name: 'Ekonomi', words: ['Borsa', 'Faiz', 'Enflasyon', 'Döviz', 'Bütçe', 'Yatırım'] },
    { name: 'Uzay Araçları', words: ['Roket', 'Kapsül', 'Teleskop', 'Astronot', 'Rampa', 'Modül'] },
    { name: 'Dondurma', words: ['Karamel', 'Nane', 'Tarçınlı', 'Fıstık', 'Frambuaz', 'Kayısı'] },
    { name: 'Kumaş', words: ['İpek', 'Pamuk', 'Keten', 'Yün', 'Kadife', 'Denim'] },
    { name: 'Hukuk', words: ['Hakim', 'Avukat', 'Mahkeme', 'Dava', 'Yasa', 'Jüri'] },
    { name: 'Film Türleri', words: ['Aksiyon', 'Komedi', 'Gerilim', 'Dram', 'Belgesel', 'Animasyon'] },
    { name: 'Kozmetik', words: ['Ruj', 'Fondöten', 'Maskara', 'Allık', 'Oje', 'Parfüm'] },
    { name: 'Balık', words: ['Levrek', 'Hamsi', 'Palamut', 'Çipura', 'Somon', 'Alabalık'] },
    { name: 'Ekmek', words: ['Somun', 'Çavdar', 'Lavaş', 'Bazlama', 'Pogaça', 'Francala'] },
    { name: 'Tabiat Olayları', words: ['Deprem', 'Sel', 'Fırtına', 'Tsunami', 'Heyelan', 'Kasırga'] },
    { name: 'Renk Tonları', words: ['Turkuaz', 'Bordo', 'Lacivert', 'Bej', 'Lila', 'Mercan'] },
    { name: 'İstanbul', words: ['Beyoğlu', 'Galata', 'Sultanahmet', 'Kadıköy', 'Üsküdar', 'Taksim'] },
    { name: 'Atıştırmalık', words: ['Cips', 'Patlak mısır', 'Bisküvi', 'Kuruyemiş', 'Gofret', 'Kraker'] },
    { name: 'Meşrubat', words: ['Kola', 'Soda', 'Şalgam', 'Boza', 'Komposto', 'Gazoz'] },
  ],

  en: [
    { name: 'Fruits', words: ['Apple', 'Pear', 'Cherry', 'Orange', 'Strawberry', 'Banana', 'Watermelon', 'Grape', 'Peach', 'Pineapple', 'Pomegranate', 'Fig'] },
    { name: 'Animals', words: ['Cat', 'Dog', 'Bird', 'Fish', 'Rabbit', 'Horse', 'Turtle', 'Frog', 'Lion', 'Elephant', 'Giraffe', 'Penguin'] },
    { name: 'Colors', words: ['Red', 'Blue', 'Green', 'Yellow', 'Orange', 'Purple', 'Pink', 'White'] },
    { name: 'Sports', words: ['Football', 'Basketball', 'Tennis', 'Swimming', 'Volleyball', 'Boxing', 'Cycling', 'Running'] },
    { name: 'Drinks', words: ['Tea', 'Coffee', 'Water', 'Milk', 'Lemonade', 'Juice', 'Smoothie', 'Soda'] },
    { name: 'Trees', words: ['Pine', 'Oak', 'Poplar', 'Walnut', 'Olive', 'Palm'] },
    { name: 'Music', words: ['Guitar', 'Piano', 'Drums', 'Violin', 'Flute', 'Trumpet'] },
    { name: 'Vegetables', words: ['Tomato', 'Pepper', 'Carrot', 'Potato', 'Onion', 'Eggplant', 'Broccoli', 'Corn'] },
    { name: 'Countries', words: ['Turkey', 'Japan', 'Brazil', 'Italy', 'France', 'Germany'] },
    { name: 'Planets', words: ['Mars', 'Jupiter', 'Saturn', 'Venus', 'Mercury', 'Neptune'] },
    { name: 'Seasons', words: ['Spring', 'Summer', 'Autumn', 'Winter'] },
    { name: 'Emotions', words: ['Happiness', 'Sadness', 'Surprise', 'Fear', 'Anger', 'Excitement'] },
    { name: 'School', words: ['Pencil', 'Notebook', 'Eraser', 'Ruler', 'Book', 'Backpack'] },
    { name: 'Food', words: ['Pizza', 'Hamburger', 'Sushi', 'Pasta', 'Ice cream', 'Cake'] },
    { name: 'Weather', words: ['Sunny', 'Rainy', 'Snowy', 'Cloudy', 'Stormy', 'Foggy'] },
    { name: 'Transport', words: ['Car', 'Bus', 'Train', 'Plane', 'Ship'] },
    { name: 'Clothes', words: ['Shirt', 'Pants', 'Hat', 'Shoes', 'Dress', 'Socks'] },
    { name: 'Birds', words: ['Parrot', 'Sparrow', 'Eagle', 'Seagull', 'Owl', 'Flamingo'] },
    { name: 'Flowers', words: ['Rose', 'Daisy', 'Tulip', 'Violet', 'Orchid', 'Lily'] },
    { name: 'Ocean', words: ['Dolphin', 'Shark', 'Octopus', 'Crab', 'Seahorse', 'Lobster'] },
    { name: 'Jobs', words: ['Doctor', 'Teacher', 'Engineer', 'Chef', 'Pilot', 'Police'] },
    { name: 'Furniture', words: ['Table', 'Chair', 'Closet', 'Couch', 'Bed', 'Mirror'] },
    { name: 'Body', words: ['Eye', 'Nose', 'Ear', 'Hand', 'Foot', 'Heart'] },
    { name: 'Space', words: ['Moon', 'Sun', 'Star', 'Galaxy', 'Meteor', 'Satellite'] },
    { name: 'Cities', words: ['London', 'New York', 'Tokyo', 'Paris', 'Sydney', 'Dubai'] },
    { name: 'Jewelry', words: ['Ring', 'Necklace', 'Bracelet', 'Earring', 'Brooch', 'Watch'] },
    { name: 'Desserts', words: ['Brownie', 'Cookie', 'Pudding', 'Muffin', 'Donut', 'Pie'] },
    { name: 'Spices', words: ['Salt', 'Pepper', 'Cumin', 'Thyme', 'Cinnamon', 'Saffron'] },
    { name: 'Technology', words: ['Phone', 'Tablet', 'Laptop', 'Robot', 'Drone', 'Printer'] },
    { name: 'Computer', words: ['Keyboard', 'Mouse', 'Screen', 'Processor', 'Memory', 'Disk'] },
    { name: 'Cinema', words: ['Film', 'Director', 'Actor', 'Scene', 'Script', 'Camera'] },
    { name: 'Kitchen', words: ['Pot', 'Pan', 'Knife', 'Plate', 'Glass', 'Spoon'] },
    { name: 'Nature', words: ['Mountain', 'Forest', 'River', 'Lake', 'Waterfall', 'Cave'] },
    { name: 'Vacation', words: ['Beach', 'Hotel', 'Suitcase', 'Passport', 'Sunscreen', 'Map'] },
    { name: 'Toys', words: ['Doll', 'Car', 'Lego', 'Puzzle', 'Ball', 'Kite'] },
    { name: 'Math', words: ['Addition', 'Subtraction', 'Multiply', 'Division', 'Triangle', 'Circle'] },
    { name: 'Art', words: ['Painting', 'Sculpture', 'Brush', 'Canvas', 'Paint', 'Museum'] },
    { name: 'Tools', words: ['Hammer', 'Screwdriver', 'Pliers', 'Saw', 'Drill', 'Wrench'] },
  ],

  de: [
    { name: 'Früchte', words: ['Apfel', 'Birne', 'Kirsche', 'Orange', 'Erdbeere', 'Banane', 'Wassermelone', 'Traube', 'Pfirsich', 'Ananas'] },
    { name: 'Tiere', words: ['Katze', 'Hund', 'Vogel', 'Fisch', 'Kaninchen', 'Pferd', 'Schildkröte', 'Frosch', 'Löwe', 'Elefant'] },
    { name: 'Farben', words: ['Rot', 'Blau', 'Grün', 'Gelb', 'Orange', 'Lila', 'Rosa', 'Weiß'] },
    { name: 'Sport', words: ['Fußball', 'Basketball', 'Tennis', 'Schwimmen', 'Volleyball', 'Boxen', 'Radfahren', 'Laufen'] },
    { name: 'Getränke', words: ['Tee', 'Kaffee', 'Wasser', 'Milch', 'Limonade', 'Saft', 'Smoothie', 'Cola'] },
    { name: 'Bäume', words: ['Kiefer', 'Eiche', 'Pappel', 'Walnuss', 'Olive', 'Palme'] },
    { name: 'Musik', words: ['Gitarre', 'Klavier', 'Schlagzeug', 'Geige', 'Flöte', 'Trompete'] },
    { name: 'Gemüse', words: ['Tomate', 'Paprika', 'Karotte', 'Kartoffel', 'Zwiebel', 'Aubergine', 'Brokkoli', 'Mais'] },
    { name: 'Länder', words: ['Türkei', 'Japan', 'Brasilien', 'Italien', 'Frankreich', 'Deutschland'] },
    { name: 'Planeten', words: ['Mars', 'Jupiter', 'Saturn', 'Venus', 'Merkur', 'Neptun'] },
    { name: 'Jahreszeiten', words: ['Frühling', 'Sommer', 'Herbst', 'Winter'] },
    { name: 'Gefühle', words: ['Freude', 'Traurigkeit', 'Überraschung', 'Angst', 'Wut', 'Aufregung'] },
    { name: 'Schule', words: ['Bleistift', 'Heft', 'Radiergummi', 'Lineal', 'Buch', 'Rucksack'] },
    { name: 'Essen', words: ['Pizza', 'Hamburger', 'Sushi', 'Nudeln', 'Eiscreme', 'Kuchen'] },
    { name: 'Wetter', words: ['Sonnig', 'Regnerisch', 'Schnee', 'Bewölkt', 'Stürmisch', 'Neblig'] },
    { name: 'Verkehr', words: ['Auto', 'Bus', 'Zug', 'Flugzeug', 'Schiff'] },
    { name: 'Kleidung', words: ['Hemd', 'Hose', 'Hut', 'Schuhe', 'Kleid', 'Socken'] },
    { name: 'Vögel', words: ['Papagei', 'Spatz', 'Adler', 'Möwe', 'Eule', 'Flamingo'] },
    { name: 'Blumen', words: ['Rose', 'Gänseblümchen', 'Tulpe', 'Veilchen', 'Orchidee', 'Lilie'] },
    { name: 'Meer', words: ['Delfin', 'Hai', 'Krake', 'Krabbe', 'Seepferdchen', 'Hummer'] },
    { name: 'Berufe', words: ['Arzt', 'Lehrer', 'Ingenieur', 'Koch', 'Pilot', 'Polizist'] },
    { name: 'Möbel', words: ['Tisch', 'Stuhl', 'Schrank', 'Sofa', 'Bett', 'Spiegel'] },
    { name: 'Körper', words: ['Auge', 'Nase', 'Ohr', 'Hand', 'Fuß', 'Herz'] },
    { name: 'Weltraum', words: ['Mond', 'Sonne', 'Stern', 'Galaxie', 'Meteor', 'Satellit'] },
  ],

  fr: [
    { name: 'Fruits', words: ['Pomme', 'Poire', 'Cerise', 'Orange', 'Fraise', 'Banane', 'Pastèque', 'Raisin', 'Pêche', 'Ananas'] },
    { name: 'Animaux', words: ['Chat', 'Chien', 'Oiseau', 'Poisson', 'Lapin', 'Cheval', 'Tortue', 'Grenouille', 'Lion', 'Éléphant'] },
    { name: 'Couleurs', words: ['Rouge', 'Bleu', 'Vert', 'Jaune', 'Orange', 'Violet', 'Rose', 'Blanc'] },
    { name: 'Sports', words: ['Football', 'Basket', 'Tennis', 'Natation', 'Volley', 'Boxe', 'Cyclisme', 'Course'] },
    { name: 'Boissons', words: ['Thé', 'Café', 'Eau', 'Lait', 'Limonade', 'Jus', 'Smoothie', 'Soda'] },
    { name: 'Arbres', words: ['Pin', 'Chêne', 'Peuplier', 'Noyer', 'Olivier', 'Palmier'] },
    { name: 'Musique', words: ['Guitare', 'Piano', 'Batterie', 'Violon', 'Flûte', 'Trompette'] },
    { name: 'Légumes', words: ['Tomate', 'Poivron', 'Carotte', 'Pomme de terre', 'Oignon', 'Aubergine', 'Brocoli', 'Maïs'] },
    { name: 'Pays', words: ['Turquie', 'Japon', 'Brésil', 'Italie', 'France', 'Allemagne'] },
    { name: 'Planètes', words: ['Mars', 'Jupiter', 'Saturne', 'Vénus', 'Mercure', 'Neptune'] },
    { name: 'Saisons', words: ['Printemps', 'Été', 'Automne', 'Hiver'] },
    { name: 'Émotions', words: ['Joie', 'Tristesse', 'Surprise', 'Peur', 'Colère', 'Excitation'] },
    { name: 'École', words: ['Crayon', 'Cahier', 'Gomme', 'Règle', 'Livre', 'Sac à dos'] },
    { name: 'Nourriture', words: ['Pizza', 'Hamburger', 'Sushi', 'Pâtes', 'Glace', 'Gâteau'] },
    { name: 'Météo', words: ['Ensoleillé', 'Pluvieux', 'Neigeux', 'Nuageux', 'Orageux', 'Brumeux'] },
    { name: 'Transport', words: ['Voiture', 'Bus', 'Train', 'Avion', 'Bateau'] },
    { name: 'Vêtements', words: ['Chemise', 'Pantalon', 'Chapeau', 'Chaussures', 'Robe', 'Chaussettes'] },
    { name: 'Oiseaux', words: ['Perroquet', 'Moineau', 'Aigle', 'Mouette', 'Hibou', 'Flamant'] },
    { name: 'Fleurs', words: ['Rose', 'Marguerite', 'Tulipe', 'Violette', 'Orchidée', 'Lys'] },
    { name: 'Océan', words: ['Dauphin', 'Requin', 'Pieuvre', 'Crabe', 'Hippocampe', 'Homard'] },
    { name: 'Métiers', words: ['Médecin', 'Professeur', 'Ingénieur', 'Cuisinier', 'Pilote', 'Policier'] },
    { name: 'Meubles', words: ['Table', 'Chaise', 'Armoire', 'Canapé', 'Lit', 'Miroir'] },
  ],

  es: [
    { name: 'Frutas', words: ['Manzana', 'Pera', 'Cereza', 'Naranja', 'Fresa', 'Plátano', 'Sandía', 'Uva', 'Melocotón', 'Piña'] },
    { name: 'Animales', words: ['Gato', 'Perro', 'Pájaro', 'Pez', 'Conejo', 'Caballo', 'Tortuga', 'Rana', 'León', 'Elefante'] },
    { name: 'Colores', words: ['Rojo', 'Azul', 'Verde', 'Amarillo', 'Naranja', 'Morado', 'Rosa', 'Blanco'] },
    { name: 'Deportes', words: ['Fútbol', 'Baloncesto', 'Tenis', 'Natación', 'Voleibol', 'Boxeo', 'Ciclismo', 'Correr'] },
    { name: 'Bebidas', words: ['Té', 'Café', 'Agua', 'Leche', 'Limonada', 'Zumo', 'Batido', 'Refresco'] },
    { name: 'Árboles', words: ['Pino', 'Roble', 'Álamo', 'Nogal', 'Olivo', 'Palmera'] },
    { name: 'Música', words: ['Guitarra', 'Piano', 'Batería', 'Violín', 'Flauta', 'Trompeta'] },
    { name: 'Verduras', words: ['Tomate', 'Pimiento', 'Zanahoria', 'Patata', 'Cebolla', 'Berenjena', 'Brócoli', 'Maíz'] },
    { name: 'Países', words: ['Turquía', 'Japón', 'Brasil', 'Italia', 'Francia', 'Alemania'] },
    { name: 'Planetas', words: ['Marte', 'Júpiter', 'Saturno', 'Venus', 'Mercurio', 'Neptuno'] },
    { name: 'Estaciones', words: ['Primavera', 'Verano', 'Otoño', 'Invierno'] },
    { name: 'Emociones', words: ['Alegría', 'Tristeza', 'Sorpresa', 'Miedo', 'Ira', 'Emoción'] },
    { name: 'Escuela', words: ['Lápiz', 'Cuaderno', 'Borrador', 'Regla', 'Libro', 'Mochila'] },
    { name: 'Comida', words: ['Pizza', 'Hamburguesa', 'Sushi', 'Pasta', 'Helado', 'Pastel'] },
    { name: 'Clima', words: ['Soleado', 'Lluvioso', 'Nevado', 'Nublado', 'Tormentoso', 'Nebuloso'] },
    { name: 'Transporte', words: ['Coche', 'Autobús', 'Tren', 'Avión', 'Barco'] },
    { name: 'Ropa', words: ['Camisa', 'Pantalón', 'Sombrero', 'Zapatos', 'Vestido', 'Calcetines'] },
    { name: 'Aves', words: ['Loro', 'Gorrión', 'Águila', 'Gaviota', 'Búho', 'Flamenco'] },
    { name: 'Flores', words: ['Rosa', 'Margarita', 'Tulipán', 'Violeta', 'Orquídea', 'Lirio'] },
    { name: 'Océano', words: ['Delfín', 'Tiburón', 'Pulpo', 'Cangrejo', 'Caballito', 'Langosta'] },
    { name: 'Profesiones', words: ['Doctor', 'Maestro', 'Ingeniero', 'Cocinero', 'Piloto', 'Policía'] },
    { name: 'Muebles', words: ['Mesa', 'Silla', 'Armario', 'Sofá', 'Cama', 'Espejo'] },
  ],

  ar: [
    { name: 'فواكه', words: ['تفاحة', 'كمثرى', 'كرز', 'برتقال', 'فراولة', 'موز', 'بطيخ', 'عنب', 'خوخ', 'أناناس'] },
    { name: 'حيوانات', words: ['قطة', 'كلب', 'طائر', 'سمكة', 'أرنب', 'حصان', 'سلحفاة', 'ضفدع', 'أسد', 'فيل'] },
    { name: 'ألوان', words: ['أحمر', 'أزرق', 'أخضر', 'أصفر', 'برتقالي', 'بنفسجي', 'وردي', 'أبيض'] },
    { name: 'رياضة', words: ['كرة قدم', 'كرة سلة', 'تنس', 'سباحة', 'كرة طائرة', 'ملاكمة', 'دراجة', 'جري'] },
    { name: 'مشروبات', words: ['شاي', 'قهوة', 'ماء', 'حليب', 'ليمونادة', 'عصير', 'سموذي', 'مشروب غازي'] },
    { name: 'أشجار', words: ['صنوبر', 'بلوط', 'حور', 'جوز', 'زيتون', 'نخيل'] },
    { name: 'موسيقى', words: ['جيتار', 'بيانو', 'طبول', 'كمان', 'ناي', 'بوق'] },
    { name: 'خضروات', words: ['طماطم', 'فلفل', 'جزر', 'بطاطس', 'بصل', 'باذنجان', 'بروكلي', 'ذرة'] },
    { name: 'دول', words: ['تركيا', 'اليابان', 'البرازيل', 'إيطاليا', 'فرنسا', 'ألمانيا'] },
    { name: 'كواكب', words: ['المريخ', 'المشتري', 'زحل', 'الزهرة', 'عطارد', 'نبتون'] },
    { name: 'فصول', words: ['ربيع', 'صيف', 'خريف', 'شتاء'] },
    { name: 'مشاعر', words: ['سعادة', 'حزن', 'مفاجأة', 'خوف', 'غضب', 'حماس'] },
    { name: 'مدرسة', words: ['قلم', 'دفتر', 'ممحاة', 'مسطرة', 'كتاب', 'حقيبة'] },
    { name: 'طعام', words: ['بيتزا', 'برغر', 'سوشي', 'معكرونة', 'آيس كريم', 'كعكة'] },
    { name: 'طقس', words: ['مشمس', 'ممطر', 'ثلجي', 'غائم', 'عاصف', 'ضبابي'] },
    { name: 'مواصلات', words: ['سيارة', 'حافلة', 'قطار', 'طائرة', 'سفينة'] },
    { name: 'ملابس', words: ['قميص', 'بنطلون', 'قبعة', 'حذاء', 'فستان', 'جوارب'] },
    { name: 'طيور', words: ['ببغاء', 'عصفور', 'نسر', 'نورس', 'بومة', 'فلامنغو'] },
    { name: 'زهور', words: ['وردة', 'أقحوان', 'توليب', 'بنفسج', 'أوركيد', 'زنبق'] },
    { name: 'بحر', words: ['دلفين', 'قرش', 'أخطبوط', 'سلطعون', 'حصان بحر', 'كركند'] },
  ],
};

// Emoji mapping per language (same emojis, different words)
export const EMOJI_MAP = {
  // Fruits
  'Elma': '🍏', 'Apple': '🍏', 'Apfel': '🍏', 'Pomme': '🍏', 'Manzana': '🍏', 'تفاحة': '🍏',
  'Armut': '🍐', 'Pear': '🍐', 'Birne': '🍐', 'Poire': '🍐', 'Pera': '🍐', 'كمثرى': '🍐',
  'Kiraz': '🍒', 'Cherry': '🍒', 'Kirsche': '🍒', 'Cerise': '🍒', 'Cereza': '🍒', 'كرز': '🍒',
  'Portakal': '🍊', 'Orange': '🍊', 'Naranja': '🍊', 'برتقال': '🍊',
  'Çilek': '🍓', 'Strawberry': '🍓', 'Erdbeere': '🍓', 'Fraise': '🍓', 'Fresa': '🍓', 'فراولة': '🍓',
  'Muz': '🍌', 'Banana': '🍌', 'Banane': '🍌', 'Plátano': '🍌', 'موز': '🍌',
  'Karpuz': '🍉', 'Watermelon': '🍉', 'Wassermelone': '🍉', 'Pastèque': '🍉', 'Sandía': '🍉', 'بطيخ': '🍉',
  'Üzüm': '🍇', 'Grape': '🍇', 'Traube': '🍇', 'Raisin': '🍇', 'Uva': '🍇', 'عنب': '🍇',
  'Şeftali': '🍑', 'Peach': '🍑', 'Pfirsich': '🍑', 'Pêche': '🍑', 'Melocotón': '🍑', 'خوخ': '🍑',
  'Ananas': '🍍', 'Pineapple': '🍍', 'أناناس': '🍍',
  'Nar': '❤️‍🔥', 'Pomegranate': '🔴', 'İncir': '🍠', 'Fig': '🍈',
  // Animals
  'Kedi': '🐱', 'Cat': '🐱', 'Katze': '🐱', 'Chat': '🐱', 'Gato': '🐱', 'قطة': '🐱',
  'Köpek': '🐶', 'Dog': '🐶', 'Hund': '🐶', 'Chien': '🐶', 'Perro': '🐶', 'كلب': '🐶',
  'Kuş': '🐦', 'Bird': '🐦', 'Vogel': '🐦', 'Oiseau': '🐦', 'Pájaro': '🐦', 'طائر': '🐦',
  'Balık': '🐟', 'Fish': '🐟', 'Fisch': '🐟', 'Poisson': '🐟', 'Pez': '🐟', 'سمكة': '🐟',
  'Tavşan': '🐰', 'Rabbit': '🐰', 'Kaninchen': '🐰', 'Lapin': '🐰', 'Conejo': '🐰', 'أرنب': '🐰',
  'At': '🐴', 'Horse': '🐴', 'Pferd': '🐴', 'Cheval': '🐴', 'Caballo': '🐴', 'حصان': '🐴',
  'Kaplumbağa': '🐢', 'Turtle': '🐢', 'Schildkröte': '🐢', 'Tortue': '🐢', 'Tortuga': '🐢', 'سلحفاة': '🐢',
  'Kurbağa': '🐸', 'Frog': '🐸', 'Frosch': '🐸', 'Grenouille': '🐸', 'Rana': '🐸', 'ضفدع': '🐸',
  'Aslan': '🦁', 'Lion': '🦁', 'Löwe': '🦁', 'León': '🦁', 'أسد': '🦁',
  'Fil': '🐘', 'Elephant': '🐘', 'Elefant': '🐘', 'Éléphant': '🐘', 'Elefante': '🐘', 'فيل': '🐘',
  'Zürafa': '🦒', 'Giraffe': '🦒', 'Girafe': '🦒', 'Jirafa': '🦒',
  'Penguen': '🐧', 'Penguin': '🐧', 'Pingouin': '🐧', 'Pingüino': '🐧',
  // Colors
  'Kırmızı': '🔴', 'Red': '🔴', 'Rot': '🔴', 'Rouge': '🔴', 'Rojo': '🔴', 'أحمر': '🔴',
  'Mavi': '🔵', 'Blue': '🔵', 'Blau': '🔵', 'Bleu': '🔵', 'Azul': '🔵', 'أزرق': '🔵',
  'Yeşil': '🟢', 'Green': '🟢', 'Grün': '🟢', 'Vert': '🟢', 'Verde': '🟢', 'أخضر': '🟢',
  'Sarı': '🟡', 'Yellow': '🟡', 'Gelb': '🟡', 'Jaune': '🟡', 'Amarillo': '🟡', 'أصفر': '🟡',
  'Turuncu': '🟠', 'برتقالي': '🟠',
  'Mor': '🟣', 'Purple': '🟣', 'Lila': '🟣', 'Violet': '🟣', 'Morado': '🟣', 'بنفسجي': '🟣',
  'Pembe': '💗', 'Pink': '💗', 'Rosa': '💗', 'Rose': '💗', 'وردي': '💗',
  'Beyaz': '⚪', 'White': '⚪', 'Weiß': '⚪', 'Blanc': '⚪', 'Blanco': '⚪', 'أبيض': '⚪',
  // Sports
  'Futbol': '⚽', 'Football': '⚽', 'Fußball': '⚽', 'Fútbol': '⚽', 'كرة قدم': '⚽',
  'Basketbol': '🏀', 'Basketball': '🏀', 'Basket': '🏀', 'Baloncesto': '🏀', 'كرة سلة': '🏀',
  'Tenis': '🎾', 'Tennis': '🎾', 'تنس': '🎾',
  'Yüzme': '🏊', 'Swimming': '🏊', 'Schwimmen': '🏊', 'Natation': '🏊', 'Natación': '🏊', 'سباحة': '🏊',
  'Voleybol': '🏐', 'Volleyball': '🏐', 'Volley': '🏐', 'Voleibol': '🏐', 'كرة طائرة': '🏐',
  'Boks': '🥊', 'Boxing': '🥊', 'Boxen': '🥊', 'Boxe': '🥊', 'Boxeo': '🥊', 'ملاكمة': '🥊',
  'Bisiklet': '🚴', 'Cycling': '🚴', 'Radfahren': '🚴', 'Cyclisme': '🚴', 'Ciclismo': '🚴', 'دراجة': '🚴',
  'Koşu': '🏃', 'Running': '🏃', 'Laufen': '🏃', 'Course': '🏃', 'Correr': '🏃', 'جري': '🏃',
  // Drinks
  'Çay': '🍵', 'Tea': '🍵', 'Tee': '🍵', 'Thé': '🍵', 'Té': '🍵', 'شاي': '🍵',
  'Kahve': '☕', 'Coffee': '☕', 'Kaffee': '☕', 'Café': '☕', 'قهوة': '☕',
  'Su': '💧', 'Water': '💧', 'Wasser': '💧', 'Eau': '💧', 'Agua': '💧', 'ماء': '💧',
  'Süt': '🥛', 'Milk': '🥛', 'Milch': '🥛', 'Lait': '🥛', 'Leche': '🐄', 'حليب': '🐄',
  'Limonata': '🍋', 'Lemonade': '🍋', 'Limonade': '🍋', 'ليمونادة': '🍋',
  'Ayran': '🥃', 'Juice': '🧃', 'Saft': '🧃', 'Jus': '🧃', 'Zumo': '🧃', 'عصير': '🧃',
  'Meyve suyu': '🧃', 'Smoothie': '🧋', 'Soda': '🫧', 'Cola': '🥫', 'Refresco': '🧊', 'مشروب غازي': '🧊',
  // Trees
  'Çam': '🌲', 'Pine': '🌲', 'Kiefer': '🌲', 'Pin': '🌲', 'Pino': '🌲', 'صنوبر': '🌲',
  'Meşe': '🌳', 'Oak': '🌿', 'Eiche': '🌿', 'Chêne': '🌳', 'Roble': '🌿', 'بلوط': '🌿',
  'Kavak': '🎋', 'Poplar': '🌾', 'Pappel': '🌾', 'Peuplier': '🌾', 'Álamo': '🌾', 'حور': '🌾',
  'Ceviz': '🥜', 'Walnut': '🥜', 'Walnuss': '🥜', 'Noyer': '🥜', 'Nogal': '🥜', 'جوز': '🥜',
  'Zeytin': '🫒', 'Olive': '🥒', 'Olivier': '🥒', 'Olivo': '🥒', 'زيتون': '🥒',
  'Palmiye': '🌴', 'Palm': '🌴', 'Palme': '🌴', 'Palmier': '🌴', 'Palmera': '🌴', 'نخيل': '🌴',
  // Music
  'Gitar': '🎸', 'Guitar': '🎸', 'Gitarre': '🎸', 'Guitare': '🎸', 'Guitarra': '🎸', 'جيتار': '🎸',
  'Piyano': '🎹', 'Piano': '🎹', 'Klavier': '🎹', 'بيانو': '🎹',
  'Davul': '🥁', 'Drums': '🥁', 'Schlagzeug': '🥁', 'Batterie': '🥁', 'Batería': '🥁', 'طبول': '🥁',
  'Keman': '🎻', 'Violin': '🎻', 'Geige': '🎻', 'Violon': '🎻', 'Violín': '🎻', 'كمان': '🎻',
  'Flüt': '🎼', 'Flute': '🎼', 'Flöte': '🎵', 'Flûte': '🎼', 'Flauta': '🎵', 'ناي': '🎵',
  'Saz': '🪕', 'Trumpet': '🎺', 'Trompete': '🎺', 'Trompette': '🎺', 'Trompeta': '🎺', 'بوق': '🎺',
  // Vegetables
  'Domates': '🍅', 'Tomato': '🍅', 'Tomate': '🍅', 'طماطم': '🍅',
  'Biber': '🌶️', 'Pepper': '🥜', 'Paprika': '🌶️', 'Poivron': '🌶️', 'Pimiento': '🌶️', 'فلفل': '🌶️',
  'Havuç': '🥕', 'Carrot': '🥕', 'Karotte': '🥕', 'Carotte': '🥕', 'Zanahoria': '🥕', 'جزر': '🥕',
  'Patates': '🥔', 'Potato': '🥔', 'Kartoffel': '🥔', 'Pomme de terre': '🥔', 'Patata': '🥔', 'بطاطس': '🥔',
  'Soğan': '🧅', 'Onion': '🧅', 'Zwiebel': '🧅', 'Oignon': '🧅', 'Cebolla': '🧅', 'بصل': '🧅',
  'Patlıcan': '🍆', 'Eggplant': '🍆', 'Aubergine': '🍆', 'Berenjena': '🍆', 'باذنجان': '🍆',
  'Brokoli': '🥦', 'Broccoli': '🥦', 'Brokkoli': '🥦', 'Brocoli': '🥦', 'Brócoli': '🥦', 'بروكلي': '🥦',
  'Mısır': '🌽', 'Corn': '🌽', 'Mais': '🌽', 'Maïs': '🌽', 'Maíz': '🌽', 'ذرة': '🌽',
  // Countries
  'Türkiye': '🇹🇷', 'Turkey': '🇹🇷', 'Türkei': '🇹🇷', 'Turquie': '🇹🇷', 'Turquía': '🇹🇷', 'تركيا': '🇹🇷',
  'Japonya': '🇯🇵', 'Japan': '🇯🇵', 'Japon': '🇯🇵', 'اليابان': '🇯🇵',
  'Brezilya': '🇧🇷', 'Brazil': '🇧🇷', 'Brasilien': '🇧🇷', 'Brésil': '🇧🇷', 'البرازيل': '🇧🇷',
  'İtalya': '🇮🇹', 'Italy': '🇮🇹', 'Italien': '🇮🇹', 'Italie': '🇮🇹', 'Italia': '🇮🇹', 'إيطاليا': '🇮🇹',
  'Fransa': '🇫🇷', 'France': '🇫🇷', 'Frankreich': '🇫🇷', 'Francia': '🇫🇷', 'فرنسا': '🇫🇷',
  'Almanya': '🇩🇪', 'Germany': '🇩🇪', 'Deutschland': '🇩🇪', 'Allemagne': '🇩🇪', 'Alemania': '🇩🇪', 'ألمانيا': '🇩🇪',
  // Planets
  'Mars': '⛰️', 'Marte': '⛰️', 'المريخ': '⛰️',
  'Jüpiter': '🌕', 'Jupiter': '🌕', 'Júpiter': '🌕', 'المشتري': '🌕',
  'Satürn': '💫', 'Saturn': '💫', 'Saturne': '💫', 'Saturno': '💫', 'زحل': '💫',
  'Venüs': '⭐', 'Venus': '⭐', 'Vénus': '⭐', 'الزهرة': '⭐',
  'Merkür': '🌑', 'Mercury': '🌑', 'Merkur': '🌑', 'Mercure': '🌑', 'Mercurio': '🌑', 'عطارد': '🌑',
  'Neptün': '🌊', 'Neptune': '🌊', 'Neptun': '🌊', 'Neptuno': '🌊', 'نبتون': '🌊',
  // Seasons
  'İlkbahar': '🌸', 'Spring': '🌸', 'Frühling': '🌸', 'Printemps': '🌸', 'Primavera': '🌸', 'ربيع': '🌸',
  'Yaz': '☀️', 'Summer': '☀️', 'Sommer': '☀️', 'Été': '☀️', 'Verano': '☀️', 'صيف': '☀️',
  'Sonbahar': '🍂', 'Autumn': '🍂', 'Herbst': '🍂', 'Automne': '🍂', 'Otoño': '🍂', 'خريف': '🍂',
  'Kış': '❄️', 'Winter': '❄️', 'Hiver': '❄️', 'Invierno': '❄️', 'شتاء': '❄️',
  // Emotions
  'Mutluluk': '😊', 'Happiness': '😊', 'Freude': '😊', 'Joie': '😊', 'Alegría': '😊', 'سعادة': '😊',
  'Üzüntü': '😢', 'Sadness': '😢', 'Traurigkeit': '😢', 'Tristesse': '😢', 'Tristeza': '😢', 'حزن': '😢',
  'Şaşkınlık': '😲', 'Surprise': '😲', 'Überraschung': '😲', 'مفاجأة': '😲', 'Sorpresa': '😲',
  'Korku': '😨', 'Fear': '😨', 'Angst': '😨', 'Peur': '😨', 'Miedo': '😨', 'خوف': '😨',
  'Öfke': '😡', 'Anger': '😡', 'Wut': '😡', 'Colère': '😡', 'Ira': '😡', 'غضب': '😡',
  'Heyecan': '🤩', 'Excitement': '🤩', 'Aufregung': '🤩', 'Excitation': '🤩', 'Emoción': '🤩', 'حماس': '🤩',
  // School
  'Kalem': '✏️', 'Pencil': '✏️', 'Bleistift': '✏️', 'Crayon': '✏️', 'Lápiz': '✏️', 'قلم': '✏️',
  'Defter': '📓', 'Notebook': '📓', 'Heft': '📓', 'Cahier': '📓', 'Cuaderno': '📓', 'دفتر': '📓',
  'Silgi': '🧽', 'Eraser': '🧽', 'Radiergummi': '🧽', 'Gomme': '🧽', 'Borrador': '🧽', 'ممحاة': '🧽',
  'Cetvel': '📏', 'Ruler': '📏', 'Lineal': '📏', 'Règle': '📏', 'Regla': '📏', 'مسطرة': '📏',
  'Kitap': '📖', 'Book': '📖', 'Buch': '📖', 'Livre': '📖', 'Libro': '📖', 'كتاب': '📖',
  'Sırt çantası': '🎒', 'Backpack': '🎒', 'Rucksack': '🎒', 'Sac à dos': '🎒', 'Mochila': '🎒', 'حقيبة': '🎒',
  // Food
  'Pizza': '🍕', 'بيتزا': '🍕',
  'Hamburger': '🍔', 'Hamburguesa': '🍔', 'برغر': '🍔',
  'Sushi': '🍣', 'سوشي': '🍣',
  'Makarna': '🍝', 'Pasta': '🍝', 'Nudeln': '🍝', 'Pâtes': '🍝', 'معكرونة': '🍝',
  'Dondurma': '🍦', 'Ice cream': '🍦', 'Eiscreme': '🍦', 'Glace': '🍦', 'Helado': '🍦', 'آيس كريم': '🍦',
  'Kek': '🎂', 'Cake': '🎂', 'Kuchen': '🎂', 'Gâteau': '🎂', 'Pastel': '🎂', 'كعكة': '🎂',
  // Weather
  'Güneşli': '☀️', 'Sunny': '☀️', 'Sonnig': '☀️', 'Ensoleillé': '☀️', 'Soleado': '☀️', 'مشمس': '☀️',
  'Yağmurlu': '🌧️', 'Rainy': '🌧️', 'Regnerisch': '🌧️', 'Pluvieux': '🌧️', 'Lluvioso': '🌧️', 'ممطر': '🌧️',
  'Karlı': '🌨️', 'Snowy': '🌨️', 'Schnee': '🌨️', 'Neigeux': '🌨️', 'Nevado': '🌨️', 'ثلجي': '🌨️',
  'Bulutlu': '☁️', 'Cloudy': '☁️', 'Bewölkt': '☁️', 'Nuageux': '☁️', 'Nublado': '☁️', 'غائم': '☁️',
  'Fırtınalı': '⛈️', 'Stormy': '⛈️', 'Stürmisch': '⛈️', 'Orageux': '⛈️', 'Tormentoso': '⛈️', 'عاصف': '⛈️',
  'Sisli': '🌫️', 'Foggy': '🌫️', 'Neblig': '🌫️', 'Brumeux': '🌫️', 'Nebuloso': '🌫️', 'ضبابي': '🌫️',
  // Transport
  'Araba': '🚗', 'Car': '🚗', 'Auto': '🚗', 'Voiture': '🚗', 'Coche': '🚗', 'سيارة': '🚗',
  'Otobüs': '🚌', 'Bus': '🚌', 'Autobús': '🚌', 'حافلة': '🚌',
  'Tren': '🚆', 'Train': '🚆', 'Zug': '🚆', 'قطار': '🚆',
  'Uçak': '✈️', 'Plane': '✈️', 'Flugzeug': '✈️', 'Avion': '✈️', 'Avión': '✈️', 'طائرة': '✈️',
  'Gemi': '🚢', 'Ship': '🚢', 'Schiff': '🚢', 'Bateau': '🚢', 'Barco': '🚢', 'سفينة': '🚢',
  // Clothes
  'Gömlek': '👔', 'Shirt': '👔', 'Hemd': '👔', 'Chemise': '👔', 'Camisa': '👔', 'قميص': '👔',
  'Pantolon': '👖', 'Pants': '👖', 'Hose': '👖', 'Pantalon': '👖', 'Pantalón': '👖', 'بنطلون': '👖',
  'Şapka': '🧢', 'Hat': '🧢', 'Hut': '🧢', 'Chapeau': '🧢', 'Sombrero': '🧢', 'قبعة': '🧢',
  'Ayakkabı': '👟', 'Shoes': '👟', 'Schuhe': '👟', 'Chaussures': '👟', 'Zapatos': '👟', 'حذاء': '👟',
  'Elbise': '👗', 'Dress': '👗', 'Kleid': '👗', 'Robe': '👗', 'Vestido': '👗', 'فستان': '👗',
  'Çorap': '🧦', 'Socks': '🧦', 'Socken': '🧦', 'Chaussettes': '🧦', 'Calcetines': '🧦', 'جوارب': '🧦',
  // Birds
  'Papağan': '🦜', 'Parrot': '🦜', 'Papagei': '🦜', 'Perroquet': '🦜', 'Loro': '🦜', 'ببغاء': '🦜',
  'Serçe': '🐤', 'Sparrow': '🐤', 'Spatz': '🐤', 'Moineau': '🐤', 'Gorrión': '🐤', 'عصفور': '🐤',
  'Kartal': '🦅', 'Eagle': '🦅', 'Adler': '🦅', 'Aigle': '🦅', 'Águila': '🦅', 'نسر': '🦅',
  'Martı': '🕊️', 'Seagull': '🕊️', 'Möwe': '🕊️', 'Mouette': '🕊️', 'Gaviota': '🕊️', 'نورس': '🕊️',
  'Baykuş': '🦉', 'Owl': '🦉', 'Eule': '🦉', 'Hibou': '🦉', 'Búho': '🦉', 'بومة': '🦉',
  'Flamingo': '🦩', 'Flamant': '🦩', 'Flamenco': '🦩', 'فلامنغو': '🦩',
  // New categories
  'Gül': '🌹', 'Rose': '🌹', 'وردة': '🌹', 'Rosa': '🌹',
  'Papatya': '🌼', 'Daisy': '🌼', 'Gänseblümchen': '🌼', 'Marguerite': '🌼', 'Margarita': '🌼', 'أقحوان': '🌼',
  'Lale': '🌷', 'Tulip': '🌷', 'Tulpe': '🌷', 'Tulipe': '🌷', 'Tulipán': '🌷', 'توليب': '🌷',
  'Menekşe': '🌺', 'Violet': '🌺', 'Veilchen': '🌺', 'Violette': '🌺', 'Violeta': '🌺', 'بنفسج': '🌺',
  'Orkide': '💮', 'Orchid': '💮', 'Orchidee': '💮', 'Orchidée': '💮', 'Orquídea': '💮', 'أوركيد': '💮',
  'Sümbül': '💐', 'Lily': '💐', 'Lilie': '💐', 'Lys': '💐', 'Lirio': '💐', 'زنبق': '💐',
  'Yunus': '🐬', 'Dolphin': '🐬', 'Delfin': '🐬', 'Dauphin': '🐬', 'Delfín': '🐬', 'دلفين': '🐬',
  'Köpekbalığı': '🦈', 'Shark': '🦈', 'Hai': '🦈', 'Requin': '🦈', 'Tiburón': '🦈', 'قرش': '🦈',
  'Ahtapot': '🐙', 'Octopus': '🐙', 'Krake': '🐙', 'Pieuvre': '🐙', 'Pulpo': '🐙', 'أخطبوط': '🐙',
  'Yengeç': '🦀', 'Crab': '🦀', 'Krabbe': '🦀', 'Crabe': '🦀', 'Cangrejo': '🦀', 'سلطعون': '🦀',
  'Denizatı': '🫧', 'Seahorse': '🐡', 'Seepferdchen': '🐡', 'Hippocampe': '🐡', 'Caballito': '🦑', 'حصان بحر': '🦑',
  'İstakoz': '🦞', 'Lobster': '🦞', 'Hummer': '🦞', 'Homard': '🦞', 'Langosta': '🦞', 'كركند': '🦞',
  'Doktor': '👨‍⚕️', 'Doctor': '👨‍⚕️', 'Arzt': '👨‍⚕️', 'Médecin': '👨‍⚕️', 'دكتور': '👨‍⚕️',
  'Öğretmen': '👩‍🏫', 'Teacher': '👩‍🏫', 'Lehrer': '👩‍🏫', 'Professeur': '👩‍🏫', 'Maestro': '👩‍🏫', 'معلم': '👩‍🏫',
  'Mühendis': '👷', 'Engineer': '👷', 'Ingenieur': '👷', 'مهندس': '👷',
  'Aşçı': '👨‍🍳', 'Chef': '👨‍🍳', 'Koch': '👨‍🍳', 'Cuisinier': '👨‍🍳', 'Cocinero': '👨‍🍳', 'طاهي': '👨‍🍳',
  'Pilot': '👨‍✈️', 'طيار': '👨‍✈️',
  'Polis': '👮', 'Police': '👮', 'Polizist': '👮', 'Policier': '👮', 'Policía': '👮', 'شرطي': '👮',
  'Masa': '🗂️', 'Table': '🪑', 'Tisch': '🪑', 'Mesa': '🪑',
  'Sandalye': '💺', 'Chair': '💺', 'Stuhl': '💺', 'Chaise': '💺', 'Silla': '💺',
  'Dolap': '🗄️', 'Closet': '🗄️', 'Schrank': '🗄️', 'Armoire': '🗄️', 'Armario': '🗄️',
  'Koltuk': '🛋️', 'Couch': '🛋️', 'Sofa': '🛋️', 'Canapé': '🛋️', 'Sofá': '🛋️',
  'Yatak': '🛏️', 'Bed': '🛏️', 'Bett': '🛏️', 'Lit': '🛏️', 'Cama': '🛏️',
  'Ayna': '🪞', 'Mirror': '🖼️', 'Spiegel': '🖼️', 'Miroir': '🖼️', 'Espejo': '🖼️',
  'Göz': '👁️', 'Eye': '👁️', 'Auge': '👁️',
  'Burun': '👃', 'Nose': '👃', 'Nase': '👃',
  'Kulak': '👂', 'Ear': '👂', 'Ohr': '👂',
  'El': '✋', 'Hand': '✋',
  'Ayak': '🦶', 'Foot': '🦶', 'Fuß': '🦶',
  'Kalp': '❤️', 'Heart': '❤️', 'Herz': '❤️',
  'Ay': '🌙', 'Moon': '🌙', 'Mond': '🌙',
  'Güneş': '☀️', 'Sun': '☀️', 'Sonne': '☀️',
  'Yıldız': '⭐', 'Star': '⭐', 'Stern': '⭐',
  'Galaksi': '🌌', 'Galaxy': '🌌', 'Galaxie': '🌌',
  'Meteor': '☄️',
  'Uydu': '🛰️', 'Satellite': '🛰️', 'Satellit': '🛰️',
  // Turkish cities
  'İstanbul': '🌉', 'Ankara': '🏛️', 'İzmir': '⛵', 'Antalya': '🌅', 'Bursa': '🏔️', 'Trabzon': '⛰️',
  // English cities
  'London': '🏰', 'New York': '🗽', 'Tokyo': '🗼', 'Paris': '🥐', 'Sydney': '🌉', 'Dubai': '🏗️',
  // Jewelry
  'Yüzük': '💍', 'Ring': '💍', 'Kolye': '📿', 'Necklace': '📿', 'Bilezik': '🔗', 'Bracelet': '⌚',
  'Küpe': '👂', 'Earring': '👂', 'Broş': '🌺', 'Brooch': '📌', 'Saat': '⌚', 'Watch': '⏱️',
  // Turkish desserts
  'Baklava': '🔶', 'Künefe': '🧁', 'Sütlaç': '🍮', 'Helva': '🍬', 'Lokum': '🍡', 'Kadayıf': '🥮',
  // English desserts
  'Brownie': '🍫', 'Cookie': '🍪', 'Pudding': '🍮', 'Muffin': '🍩', 'Donut': '🥜', 'Pie': '🥧',
  // Spices
  'Tuz': '🧂', 'Salt': '🧂', 'Kimyon': '🌿', 'Cumin': '🌿', 'Kekik': '🌱', 'Thyme': '🌱',
  'Tarçın': '🍂', 'Cinnamon': '🍂', 'Safran': '✨', 'Saffron': '🌾',
  // Technology
  'Telefon': '📞', 'Phone': '📞', 'Tablet': '📲', 'Laptop': '💻', 'Robot': '🤖', 'Drone': '🛸', 'Yazıcı': '🖨️', 'Printer': '🖨️',
  // Computer
  'Klavye': '⌨️', 'Keyboard': '⌨️', 'Mouse': '🖱️', 'Ekran': '🖥', 'Screen': '🖥️', 'İşlemci': '🧠', 'Processor': '🔲', 'Bellek': '💾', 'Memory': '💾', 'Disk': '💿',
  // Cinema
  'Film': '🎞️', 'Yönetmen': '🎥', 'Director': '🎥', 'Aktör': '🎭', 'Actor': '🎭', 'Sahne': '🎪', 'Scene': '🎪', 'Senaryo': '📝', 'Script': '📝', 'Kamera': '📹', 'Camera': '📷',
  // Kitchen
  'Tencere': '🍲', 'Pot': '🍲', 'Tava': '🥘', 'Pan': '🥘', 'Bıçak': '🔪', 'Knife': '🔪', 'Tabak': '🍽️', 'Plate': '🍽️', 'Bardak': '🥃', 'Glass': '🥃', 'Kaşık': '🥄', 'Spoon': '🥄',
  // Nature
  'Dağ': '⛰️', 'Mountain': '⛰️', 'Orman': '🌲', 'Forest': '🌲', 'Nehir': '🏞️', 'River': '🏞️', 'Göl': '💧', 'Lake': '💧', 'Şelale': '💦', 'Waterfall': '💦', 'Mağara': '🕳️', 'Cave': '🕳️',
  // Vacation
  'Plaj': '⛱️', 'Beach': '🏝️', 'Otel': '🏨', 'Hotel': '🏨', 'Bavul': '💼', 'Suitcase': '🧳', 'Pasaport': '🛂', 'Passport': '🛂', 'Güneş kremi': '🧴', 'Sunscreen': '🧴', 'Harita': '🗺️', 'Map': '🗺️',
  // Toys
  'Bebek': '🧸', 'Doll': '🎎', 'Lego': '🧱', 'Puzzle': '🧩', 'Top': '🎾', 'Ball': '🏐', 'Uçurtma': '🎏', 'Kite': '🎏',
  // Math
  'Toplama': '➕', 'Addition': '➕', 'Çıkarma': '➖', 'Subtraction': '➖', 'Çarpma': '✖️', 'Multiply': '✖️', 'Bölme': '➗', 'Division': '➗', 'Üçgen': '🔺', 'Triangle': '🔺', 'Daire': '⭕', 'Circle': '⭕',
  // Art
  'Resim': '🖼️', 'Painting': '🖼️', 'Heykel': '🗿', 'Sculpture': '🗿', 'Fırça': '🖌️', 'Brush': '🖌️', 'Tuval': '🧑‍🎨', 'Canvas': '📐', 'Boya': '🎨', 'Paint': '🖍️', 'Müze': '🏛️', 'Museum': '🏛️',
  // Tools
  'Çekiç': '🔨', 'Hammer': '🔨', 'Tornavida': '🛠️', 'Screwdriver': '🛠️', 'Pense': '🔗', 'Pliers': '🔗', 'Testere': '⚙️', 'Saw': '⚙️', 'Matkap': '⛏️', 'Drill': '⛏️', 'Anahtar': '🔧', 'Wrench': '🔑',
  // Stationery
  'Makas': '✂️', 'Yapıştırıcı': '🩹', 'Bant': '🖇️', 'Zımba': '📌', 'Dosya': '📁', 'Klasör': '📂',
  // Heroes
  'Maske': '🎭', 'Pelerin': '🧥', 'Güç': '💪', 'Kalkan': '🛡️', 'Kılıç': '⚔️', 'Zırh': '⚙️',
  // Kahvaltı
  'Peynir': '🧀', 'Bal': '🍯', 'Yumurta': '🥚', 'Reçel': '🥫', 'Tereyağı': '🧈',
  // Masal
  'Prenses': '👸', 'Ejderha': '🐉', 'Büyücü': '🧙', 'Şövalye': '🤺', 'Hazine': '💎', 'Kale': '🏰',
  // Deniz Araçları
  'Tekne': '🚤', 'Yat': '🛥️', 'Feribot': '⛴️', 'Kayık': '🛶', 'Sürat': '💨', 'Yelkenli': '🚢',
  // Kış Sporları
  'Kayak': '🎿', 'Buz pateni': '⛸️', 'Hokey': '🏒', 'Snowboard': '🏂', 'Kızak': '🛷', 'Curling': '🥌',
  // Süt Ürünleri
  'Yoğurt': '🏺', 'Kaymak': '🧈', 'Kefir': '🍶',
  // Bina
  'Kapı': '🚪', 'Pencere': '🪟', 'Çatı': '🏠', 'Merdiven': '🪜', 'Balkon': '🏢', 'Asansör': '🛗',
  // Orman
  'Mantar': '🍄', 'Yaprak': '🍃', 'Sincap': '🐿️', 'Geyik': '🦌', 'Tilki': '🦊', 'Kurt': '🐺',
  // Plaj
  'Kum': '🏖️', 'Dalga': '🌊', 'Şemsiye': '☂️', 'Deniz yıldızı': '🌟', 'Kabuk': '🐚', 'Palet': '🤿',
  // Müzik Türleri
  'Pop': '🎤', 'Rock': '🎸', 'Caz': '🎷', 'Klasik': '🎻', 'Rap': '🎧', 'Blues': '🎺',
  // Trafik
  'Işık': '🚥', 'Tabela': '📋', 'Yol': '🛣️', 'Kavşak': '↕️', 'Köprü': '🌉', 'Tünel': '🚇',
  // Market
  'Sepet': '🧺', 'Kasa': '💰', 'Raf': '📦', 'Etiket': '🏷️', 'Poşet': '🛍️', 'Fatura': '🧾',
  // Hastane
  'Hemşire': '👩‍⚕️', 'İlaç': '💊', 'Ameliyat': '🏥', 'Muayene': '🔬', 'Röntgen': '🦴', 'Ambulans': '🚑',
  // Oyunlar
  'Satranç': '👑', 'Dama': '⬛', 'Tavla': '🎲', 'Bilardo': '🎱', 'Dart': '🎯', 'Bowling': '🎳',
  // Gemi
  'Kaptan': '🧑‍✈️', 'Dümen': '☸️', 'Çapa': '⚓', 'Güverte': '🛟', 'Pusula': '🧭', 'Yelken': '⛵',
  // Sabah
  'Alarm': '⏰', 'Duş': '🚿', 'Diş fırçası': '🦷', 'Gazete': '📰', 'Erken': '🌄',
  // Ağırlık Birimleri
  'Gram': '⚖️', 'Kilogram': '📊', 'Ton': '🏋️', 'Ons': '🥄', 'Miligram': '🔬', 'Libre': '🔢',
  // Partisyon / Müzik
  'Nota': '🎵', 'Akor': '🎸', 'Ritim': '🥁', 'Melodi': '🎶', 'Tempo': '⏱️', 'Armoni': '🎻',
  // Simya
  'Altın': '💰', 'Gümüş': '🥈', 'Bakır': '🟤', 'Demir': '⛓️', 'Kristal': '💠',
  // Fitness
  'Plank': '🏋️', 'Squat': '🦵', 'Dambıl': '🏋️‍♂️', 'Kardiyo': '❤️',
  // Türk Mutfağı
  'Kebap': '🍖', 'Lahmacun': '🫓', 'Pide': '🥖', 'Mantı': '🥟', 'Börek': '🥧', 'Dolma': '🍃',
  // Sinema  
  'Yönetmen': '🎬', 'Aktör': '🎭', 'Senaryo': '📝',
  // Tatil
  'Bavul': '💼', 'Güneş kremi': '🧴',
  // Böcekler
  'Kelebek': '🦋', 'Karınca': '🐜', 'Arı': '🐝', 'Uğur böceği': '🐞', 'Ateş böceği': '🐛', 'Çekirge': '🦗',
  // Kamp
  'Çadır': '🏕️', 'Uyku tulumu': '🛏️', 'Fener': '🔦', 'Balta': '⚒️',
  // Fotoğrafçılık
  'Objektif': '🔍', 'Flaş': '📸', 'Poz': '🤳', 'Kadraj': '🖼️', 'Deklanşör': '⏺️', 'Tripod': '📐',
  // Astroloji
  'Koç': '🐏', 'Boğa': '♉', 'İkizler': '♊', 'Yengeç': '🦀', 'Aslan burcu': '♌', 'Akrep': '♏',
  // Mitoloji
  'Zeus': '⚡', 'Athena': '🦉', 'Poseidon': '🔱', 'Hades': '💀', 'Apollon': '☀️', 'Artemis': '🌙',
  // Pasta
  'Vanilya': '🤎', 'Muffin': '🍩', 'Kurabiye': '🍪', 'Kek': '🎂',
  // Düğün
  'Gelin': '👰', 'Damat': '🤵', 'Nikah': '💍', 'Düvak': '👗', 'Davetiye': '💌', 'Dans': '💃',
  // Bebek
  'Emzik': '👶', 'Biberon': '🍼', 'Bez': '🧷', 'Beşik': '🛏️', 'Mama': '🥣', 'Kundak': '👣',
  // Bahçe
  'Tohum': '🥜', 'Toprak': '🟤', 'Gübre': '♻️', 'Sulama': '💧', 'Kürek': '⛏️', 'Sera': '🌱',
  // Enstrüman
  'Piyano': '🎹', 'Keman': '🎻', 'Davul': '🥁', 'Flüt': '🎼', 'Saz': '🪕',
  // Yazılım
  'Kod': '⌨️', 'Hata': '🐛', 'Sunucu': '🖥️', 'Veritabanı': '🗄️', 'Algoritma': '🧮', 'Arayüz': '📱',
  // Coğrafya
  'Vadi': '🏞️', 'Yarımada': '🏝️', 'Okyanus': '🌊', 'Çöl': '🏜️', 'Volkan': '🌋',
  // Tarih
  'İmparator': '👑', 'Antlaşma': '🤝', 'Hanedan': '🏰', 'Fetih': '🏴', 'Devrim': '✊',
  // Mimari
  'Sütun': '🏛️', 'Kemer': '🌉', 'Kubbe': '🕌', 'Minare': '🗼', 'Köşk': '🏡', 'Revak': '🧱',
  // Dövüş Sanatları
  'Karate': '👊', 'Judo': '🤸', 'Taekwondo': '🦶', 'Aikido': '🥷', 'Kung fu': '🥊', 'Boks': '🥊',
  // Çay
  'Demlik': '☕', 'Çaydanlık': '🫖', 'Poşet çay': '🍃', 'Samovar': '♨️',
  // Ekonomi
  'Borsa': '💹', 'Faiz': '💰', 'Enflasyon': '📉', 'Döviz': '💱', 'Bütçe': '🧾', 'Yatırım': '🏦',
  // Uzay Araçları
  'Roket': '🚀', 'Teleskop': '🔭', 'Astronot': '👨‍🚀', 'İstasyon': '🛰️', 'Modül': '📦',
  // Dondurma
  'Karamel': '🍮', 'Çilek': '🍓', 'Fıstık': '🥜', 'Frambuaz': '🫐', 'Kayısı': '🍑',
  // Kumaş
  'İpek': '🧣', 'Pamuk': '☁️', 'Keten': '🧶', 'Yün': '🐑', 'Kadife': '🎀', 'Denim': '👖',
  // Hukuk
  'Hakim': '👨‍⚖️', 'Avukat': '👩‍⚖️', 'Mahkeme': '🏛️', 'Dava': '📋', 'Kanun': '🎹', 'Yasa': '📜', 'Jüri': '🧑‍🤝‍🧑',
  // Film Türleri
  'Aksiyon': '💥', 'Komedi': '😂', 'Korku': '👻', 'Dram': '🎭', 'Belgesel': '🎥', 'Animasyon': '🎨',
  // Kozmetik
  'Ruj': '👄', 'Fondöten': '🧴', 'Maskara': '👁️', 'Allık': '🌸', 'Oje': '💅', 'Parfüm': '🌹',
  // Balık
  'Levrek': '🐠', 'Hamsi': '🐟', 'Palamut': '🎣', 'Çipura': '🎏', 'Somon': '🍣', 'Alabalık': '🐡',
  // Ekmek
  'Lavaş': '🥙', 'Bazlama': '🥞', 'Pogaça': '🥐', 'Francala': '🥯',
  // Tabiat Olayları
  'Deprem': '🌍', 'Sel': '🌊', 'Fırtına': '⛈️', 'Tsunami': '🌀', 'Heyelan': '⛰️', 'Kasırga': '💨',
  // Renk Tonları
  'Turkuaz': '💙', 'Bordo': '🍷', 'Lacivert': '🔵', 'Bej': '⬜', 'Lila': '🟣', 'Mercan': '🐚',
  // İstanbul
  'Galata': '🗼', 'Sultanahmet': '🕌', 'Kadıköy': '⛴️', 'Üsküdar': '🌅',
  // Atıştırmalık
  'Cips': '🍟', 'Bisküvi': '🍪', 'Kuruyemiş': '🥜', 'Gofret': '🍫', 'Kraker': '🍘',

  'Kaplan': '🐯', 'Maden suyu': '🫧', 'Çorba': '🍜', 'Pilav': '🍚',
  'Metro': '🚇', 'Konya': '🌾', 'Çayır': '🌾', 'Havuz': '🏊', 'Kukla': '🧸',
  // Meşrubat
  'Kola': '🥤', 'Şalgam': '🧃', 'Boza': '🍶', 'Limonata': '🍋', 'Gazoz': '🍾',
  // Yeni/değişen kelimeler
  'Nane': '🍃', 'Tarçınlı': '🍂', 'Güreş': '🏅', 'Soda': '🫧', 'Komposto': '🍎',
  'Ud': '🎶', 'Ney': '🎼', 'Tanbur': '🎸', 'Zurna': '📯', 'Bağlama': '🎵',
  'Gerilim': '😱', 'Fondan': '🧁', 'Pandispanya': '🍰', 'Brownie': '🍫',
  'Başak': '♍', 'Kapsül': '🛸', 'Rampa': '🏗️', 'Fincan': '🍶',
  'Somun': '🥖', 'Çavdar': '🌾', 'Matara': '💧', 'Kamp ateşi': '🔥',
  'Patlak mısır': '🍿', 'Beyoğlu': '🏙️', 'Plato': '🏔️',
  // Eksik compound emojiler
  'Ateş': '🔥', 'Savaş': '⚔️', 'Şeker': '🍬', 'Limon': '🍋', 'Taksim': '🚇',
  'Çikolata': '🍫', 'Boğaz': '🌊',
  'Uğur böceği': '🐞', 'Ateş böceği': '🐛', 'Uyku tulumu': '🛏️',
  'Aslan burcu': '♌', 'Poşet çay': '🍃', 'Kung fu': '🥊',
  // Eksik düzeltmeler
  'Elmas': '💎',
  'Simit': '🥯', 'Lor': '🧀', 'Krema': '🍦', 'Süzme': '🥣',
  'Pul biber': '🔴', 'Tost': '🍞', 'Burpee': '🏃', 'Pilates': '🧘',
  'Tren': '🚆', 'Misket': '🔵',
  // Boşluklu kelimeler
  'Meyve suyu': '🧃', 'Sırt çantası': '🎒', 'Güneş kremi': '🧴',
  'Buz pateni': '⛸️', 'Deniz yıldızı': '⭐', 'Diş fırçası': '🦷',
  // İyileştirilmiş emojiler
  'Bakır': '🟤', 'Gümüş': '🤍',
  'Ingénieur': '👷', 'Pilote': '🧑‍✈️',
  'Piña': '🍍', 'Limonada': '🍋', 'Batido': '🥛',
  'Japón': '🇯🇵', 'Brasil': '🇧🇷',
  'Ingeniero': '👷', 'Piloto': '🧑‍✈️',
  'سموذي': '🥛',
};

// Category emoji mapping
export const CATEGORY_EMOJIS = {
  // TR
  'Meyveler': '🍎', 'Hayvanlar': '🐾', 'Renkler': '🎨', 'Sporlar': '🏆',
  'İçecekler': '🥤', 'Ağaçlar': '🌳', 'Müzik': '🎵', 'Sebzeler': '🥬',
  'Ülkeler': '🌍', 'Gezegenler': '🌍', 'Mevsimler': '🍃', 'Duygular': '💭',
  'Okul': '📚', 'Yiyecekler': '🍽️', 'Hava durumu': '🌤️', 'Ulaşım': '🚂',
  'Giysiler': '👕', 'Kuşlar': '🐦', 'Çiçekler': '🌸', 'Deniz': '🐳',
  'Meslekler': '👨‍💼', 'Mobilya': '💺', 'Vücut': '❤️', 'Uzay': '🌌',
  'Şehirler': '🏙️', 'Takılar': '💍', 'Tatlılar': '🍰', 'Baharat': '🌶️',
  'Teknoloji': '📱', 'Bilgisayar': '🖥️', 'Sinema': '🎦', 'Mutfak': '🍳',
  'Doğa': '🏔️', 'Tatil': '🧳', 'Oyuncaklar': '🧸', 'Matematik': '🔢',
  'Sanat': '🖼️', 'Araçlar': '🔧', 'Kırtasiye': '📎', 'Süper Kahramanlar': '🦸',
  'Kahvaltı': '🥐', 'Türk Mutfağı': '🥙', 'Masal': '🏰', 'Deniz Araçları': '⛵',
  'Kış Sporları': '⛷️', 'Süt Ürünleri': '🥛', 'Bina': '🏢', 'Orman': '🌲',
  'Plaj': '⛱️', 'Müzik Türleri': '🎶', 'Trafik': '🚦', 'Market': '🛒',
  'Hastane': '🏥', 'Oyunlar': '♟️', 'Gemi': '🚢', 'Sabah': '🌅',
  'Ağırlık Birimleri': '🏋️', 'Partisyon': '🎼', 'Simya': '⚗️', 'Fitness': '💪',
  'Böcekler': '🦋', 'Kamp': '⛺', 'Fotoğrafçılık': '📷', 'Astroloji': '♈',
  'Mitoloji': '🏛️', 'Pasta': '🎂', 'Düğün': '💒', 'Bebek': '👶',
  'Bahçe': '🌱', 'Enstrüman': '🎹', 'Yazılım': '👨‍💻', 'Coğrafya': '🗺️',
  'Tarih': '📜', 'Mimari': '🏗️', 'Dövüş Sanatları': '🥋', 'Çay': '🍵',
  'Ekonomi': '📈', 'Uzay Araçları': '🚀', 'Dondurma': '🍦', 'Kumaş': '🧵',
  'Hukuk': '⚖️', 'Film Türleri': '🎞️', 'Kozmetik': '💄', 'Balık': '🐟',
  'Ekmek': '🍞', 'Tabiat Olayları': '🌪️', 'Renk Tonları': '🌈',
  'İstanbul': '🕌', 'Atıştırmalık': '🍿', 'Meşrubat': '🧊',
  // EN
  'Fruits': '🍎', 'Animals': '🐾', 'Colors': '🎨', 'Sports': '🏆',
  'Drinks': '🥤', 'Trees': '🌳', 'Music': '🎵', 'Vegetables': '🥬',
  'Countries': '🌍', 'Planets': '🌍', 'Seasons': '🍃', 'Emotions': '💭',
  'School': '📚', 'Food': '🍽️', 'Weather': '🌤️', 'Transport': '🚀',
  'Clothes': '👕', 'Birds': '🐦', 'Flowers': '🌸', 'Ocean': '🐳',
  'Jobs': '👨‍💼', 'Furniture': '💺', 'Body': '❤️', 'Space': '🚀',
  'Cities': '🏙️', 'Jewelry': '💍', 'Desserts': '🍰', 'Spices': '🌶️',
  'Technology': '📱', 'Computer': '💻', 'Cinema': '🎬', 'Kitchen': '🍳',
  'Nature': '🏔️', 'Vacation': '🏖️', 'Toys': '🧸', 'Math': '🔢',
  'Art': '🎨', 'Tools': '🔧',
  // DE
  'Früchte': '🍎', 'Tiere': '🐾', 'Farben': '🎨', 'Sport': '🏆',
  'Getränke': '🥤', 'Bäume': '🌳', 'Gemüse': '🥬',
  'Länder': '🌍', 'Planeten': '🌍', 'Jahreszeiten': '🍃', 'Gefühle': '💭',
  'Schule': '📚', 'Essen': '🍽️', 'Wetter': '🌤️', 'Verkehr': '🚀',
  'Kleidung': '👕', 'Vögel': '🐦', 'Blumen': '🌸', 'Meer': '🐳',
  'Berufe': '👨‍💼', 'Möbel': '💺', 'Körper': '❤️', 'Weltraum': '🚀',
  // FR
  'Animaux': '🐾', 'Couleurs': '🎨',
  'Boissons': '🥤', 'Arbres': '🌳', 'Musique': '🎵', 'Légumes': '🥬',
  'Pays': '🌍', 'Planètes': '🌍', 'Saisons': '🍃', 'Émotions': '💭',
  'École': '📚', 'Nourriture': '🍽️', 'Météo': '🌤️',
  'Vêtements': '👕', 'Oiseaux': '🐦', 'Fleurs': '🌸', 'Océan': '🐳',
  'Métiers': '👨‍💼', 'Meubles': '💺',
  // ES
  'Frutas': '🍎', 'Animales': '🐾', 'Colores': '🎨', 'Deportes': '🏆',
  'Bebidas': '🥤', 'Árboles': '🌳', 'Música': '🎵', 'Verduras': '🥬',
  'Países': '🌍', 'Planetas': '🌍', 'Estaciones': '🍃', 'Emociones': '💭',
  'Escuela': '📚', 'Comida': '🍽️', 'Clima': '🌤️', 'Transporte': '🚀',
  'Ropa': '👕', 'Aves': '🐦', 'Flores': '🌸', 'Profesiones': '👨‍💼',
  // AR
  'فواكه': '🍎', 'حيوانات': '🐾', 'ألوان': '🎨', 'رياضة': '🏆',
  'مشروبات': '🥤', 'أشجار': '🌳', 'موسيقى': '🎵', 'خضروات': '🥬',
  'دول': '🌍', 'كواكب': '🌍', 'فصول': '🍃', 'مشاعر': '💭',
  'مدرسة': '📚', 'طعام': '🍽️', 'طقس': '🌤️', 'مواصلات': '🚀',
  'ملابس': '👕', 'طيور': '🐦', 'زهور': '🌸', 'بحر': '🐳',
};
